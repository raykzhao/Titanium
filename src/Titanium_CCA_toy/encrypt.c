...
int crypto_encrypt_keypair(unsigned char *pk, unsigned char *sk, const unsigned char *randomness)
{
	...
	for (i = 0; i < T; i++)
	{
		...
		memset(a[i] + N_act, 0, sizeof(uint32_t) * (N - N_act)); /* padding with 0 */
	...
	for (i = 0; i < T; i++)
	{
		...
		for (j = 0; j < N + D + K + 1; j++)
		{
			a[i][j] = barrett((uint64_t)a[i][j] * (uint64_t)s[j], BARRETT_FACTOR_4Q2, BARRETT_BITSHIFT_4Q2);
		}
...
int crypto_encrypt(unsigned char *c, unsigned long long *clen, const unsigned char *m, unsigned long long mlen, const unsigned char *pk, const unsigned char *randomness)
{
	...
	for (j = 0; j < N + K + 1; j++)
	{
		c1[j] = barrett((uint64_t)r[0][j] * (uint64_t)a[0][j] + (uint64_t)r[1][j] * (uint64_t)a[1][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c1[j] = barrett((uint64_t)c1[j] + (uint64_t)r[2][j] * (uint64_t)a[2][j] + (uint64_t)r[3][j] * (uint64_t)a[3][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c1[j] = barrett((uint64_t)c1[j] + (uint64_t)r[4][j] * (uint64_t)a[4][j] + (uint64_t)r[5][j] * (uint64_t)a[5][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c1[j] = barrett((uint64_t)c1[j] + (uint64_t)r[6][j] * (uint64_t)a[6][j] + (uint64_t)r[7][j] * (uint64_t)a[7][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c1[j] = barrett((uint64_t)c1[j] + (uint64_t)r[8][j] * (uint64_t)a[8][j] + (uint64_t)r[9][j] * (uint64_t)a[9][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
	}
	
	/* c2 <-- sum(r_i mp_d b_i) + mu */
	for (j = 0; j < D + K + 1; j++)
	{
		c2[j] = barrett((uint64_t)r[0][mapping_r[j]] * (uint64_t)b[0][j] + (uint64_t)r[1][mapping_r[j]] * (uint64_t)b[1][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r[2][mapping_r[j]] * (uint64_t)b[2][j] + (uint64_t)r[3][mapping_r[j]] * (uint64_t)b[3][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r[4][mapping_r[j]] * (uint64_t)b[4][j] + (uint64_t)r[5][mapping_r[j]] * (uint64_t)b[5][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r[6][mapping_r[j]] * (uint64_t)b[6][j] + (uint64_t)r[7][mapping_r[j]] * (uint64_t)b[7][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r[8][mapping_r[j]] * (uint64_t)b[8][j] + (uint64_t)r[9][mapping_r[j]] * (uint64_t)b[9][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
	}
	...
	for (i = 0; i < D; i++)
	{
		c2[i] = barrett(c2[i] + mu[i], BARRETT_FACTOR_SHORT, BARRETT_BITSHIFT_SHORT);
	}
...
int crypto_encrypt_open(unsigned char *m, unsigned long long *mlen, const unsigned char *c, unsigned long long clen, const unsigned char *sk)
{
	...
	for (i = 0; i < N + D + K + 1; i++)
	{
		c1[i] = barrett((uint64_t)c1[i] * (uint64_t)s[i], BARRETT_FACTOR_4Q2, BARRETT_BITSHIFT_4Q2);
	}
