...
int crypto_encrypt(unsigned char *c, unsigned long long *clen, const unsigned char *m, unsigned long long mlen, const unsigned char *pk, const unsigned char *randomness)
{
	...
	for (j = 0; j < D + K1 + 1; j++)
	{
		c2[j] = barrett_8q2((uint64_t)r[0][mapping_r[j]] * (uint64_t)b[0][j] + (uint64_t)r[1][mapping_r[j]] * (uint64_t)b[1][j]);
		c2[j] = barrett_8q2((uint64_t)c2[j] + (uint64_t)r[2][mapping_r[j]] * (uint64_t)b[2][j] + (uint64_t)r[3][mapping_r[j]] * (uint64_t)b[3][j]);
		c2[j] = barrett_8q2((uint64_t)c2[j] + (uint64_t)r[4][mapping_r[j]] * (uint64_t)b[4][j] + (uint64_t)r[5][mapping_r[j]] * (uint64_t)b[5][j]);
		c2[j] = barrett_8q2((uint64_t)c2[j] + (uint64_t)r[6][mapping_r[j]] * (uint64_t)b[6][j] + (uint64_t)r[7][mapping_r[j]] * (uint64_t)b[7][j]);
		c2[j] = barrett_8q2((uint64_t)c2[j] + (uint64_t)r[8][mapping_r[j]] * (uint64_t)b[8][j] + (uint64_t)r[9][mapping_r[j]] * (uint64_t)b[9][j]);
	}
