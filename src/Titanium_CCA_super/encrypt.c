...
int crypto_encrypt_keypair(unsigned char *pk, unsigned char *sk, const unsigned char *randomness)
{
	...
	for (i = 0; i < T; i++)
	{
		...
		for (j = 0; j < N + D + K + 1; j++)
		{
			a[i][j] = barrett((uint64_t)a[i][j] * (uint64_t)s[j], BARRETT_FACTOR_2Q2, BARRETT_BITSHIFT_2Q2);
		}
...
int crypto_encrypt(unsigned char *c, unsigned long long *clen, const unsigned char *m, unsigned long long mlen, const unsigned char *pk, const unsigned char *randomness)
{
	...
	for (i = 0; i < T; i++)
	{
		for (j = 0; j < N + K + 1; j++)
		{
			c1[j] = barrett((uint64_t)c1[j] + (uint64_t)(con_sub(r[i][j])) * (uint64_t)a[i][j], BARRETT_FACTOR_2Q2, BARRETT_BITSHIFT_2Q2);
		}
	}
	...
	for (i = 0; i < T; i++) 
	{
		for (j = 0; j < D + K + 1; j++)
		{
			c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r2[i][j] * (uint64_t)b[i][j], BARRETT_FACTOR_2Q2, BARRETT_BITSHIFT_2Q2); /* b(i,j) is already in [0,q-1] */
		}
	}
...
int crypto_encrypt_open(unsigned char *m, unsigned long long *mlen, const unsigned char *c, unsigned long long clen, const unsigned char *sk)
{
	...
	for (i = 0; i < N + D + K + 1; i++)
	{
		c1[i] = barrett((uint64_t)(con_sub(c1[i])) * (uint64_t)s[i], BARRETT_FACTOR_2Q2, BARRETT_BITSHIFT_2Q2);
	}
