...
int crypto_encrypt(unsigned char *c, unsigned long long *clen, const unsigned char *m, unsigned long long mlen, const unsigned char *pk, const unsigned char *randomness)
{
	...
	for (j = 0; j < D + K + 1; j++)
	{
		c2[j] = barrett((uint64_t)r2[0][j] * (uint64_t)b[0][j] + (uint64_t)r2[1][j] * (uint64_t)b[1][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r2[2][j] * (uint64_t)b[2][j] + (uint64_t)r2[3][j] * (uint64_t)b[3][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r2[4][j] * (uint64_t)b[4][j] + (uint64_t)r2[5][j] * (uint64_t)b[5][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r2[6][j] * (uint64_t)b[6][j] + (uint64_t)r2[7][j] * (uint64_t)b[7][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
		c2[j] = barrett((uint64_t)c2[j] + (uint64_t)r2[8][j] * (uint64_t)b[8][j] + (uint64_t)r2[9][j] * (uint64_t)b[9][j], BARRETT_FACTOR_8Q2, BARRETT_BITSHIFT_8Q2);
	}
