...
{
	uint64_t s[DIM_3]  __attribute__ ((aligned (32)));
	uint64_t a[T][DIM_3]  __attribute__ ((aligned (32)));
	uint64_t b[T][DIM_1]  __attribute__ ((aligned (32)));
...
int crypto_encrypt(unsigned char *c, unsigned long long *clen, const unsigned char *m, unsigned long long mlen, const unsigned char *pk)
{
	...
	uint64_t r[T][DIM_2]  __attribute__ ((aligned (32)));
	uint64_t r2[T][DIM_2]  __attribute__ ((aligned (32)));
	uint64_t a[T][DIM_2]  __attribute__ ((aligned (32)));
	uint64_t b[T][DIM_1]  __attribute__ ((aligned (32)));
	uint64_t c1[DIM_2]  __attribute__ ((aligned (32)));
	uint64_t c2[DIM_1]  __attribute__ ((aligned (32)));
	uint32_t i, j;
	unsigned char randomness[CRYPTO_RANDOMBYTES];
	__m256i u, v, t, t1;
	...
	for (j = 0; j < DIM_2; j++)
	{
		c1[j] = barrett(r[0][j] * a[0][j] + r[1][j] * a[1][j] + r[2][j] * a[2][j] + r[3][j] * a[3][j] + r[4][j] * a[4][j] + r[5][j] * a[5][j] + r[6][j] * a[6][j] + r[7][j] * a[7][j], BARRETT_FACTOR_32Q2, BARRETT_BITSHIFT_32Q2);
	}
	
	/* c2 <-- sum(r_i mp_d b_i) + mu */
	for (j = 0; j < DIM_1; j++)
	{
		c2[j] = barrett(r2[0][j] * b[0][j] + r2[1][j] * b[1][j] + r2[2][j] * b[2][j] + r2[3][j] * b[3][j] + r2[4][j] * b[4][j] + r2[5][j] * b[5][j] + r2[6][j] * b[6][j] + r2[7][j] * b[7][j], BARRETT_FACTOR_32Q2, BARRETT_BITSHIFT_32Q2);
	}
...
{
	uint64_t s[DIM_3]  __attribute__ ((aligned (32)));
	uint64_t c1[DIM_3]  __attribute__ ((aligned (32)));
