...
int crypto_encrypt(unsigned char *c, unsigned long long *clen, const unsigned char *m, unsigned long long mlen, const unsigned char *pk)
{
	...
	for (j = 0; j < DIM_2; j++)
	{
		c1[j] = barrett_32q2(r[0][j] * a[0][j] + r[1][j] * a[1][j] + r[2][j] * a[2][j] + r[3][j] * a[3][j] + r[4][j] * a[4][j] + r[5][j] * a[5][j] + r[6][j] * a[6][j] + r[7][j] * a[7][j]);
	}
	
	/* c2 <-- sum(r_i mp_d b_i) + mu */
	for (j = 0; j < DIM_1; j++)
	{
		c2[j] = barrett_32q2(r2[0][j] * b[0][j] + r2[1][j] * b[1][j] + r2[2][j] * b[2][j] + r2[3][j] * b[3][j] + r2[4][j] * b[4][j] + r2[5][j] * b[5][j] + r2[6][j] * b[6][j] + r2[7][j] * b[7][j]);
	}
